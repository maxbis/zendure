#!/usr/bin/env python3
"""
Shelly power meter reader for automate.

This module mirrors the P1 reader module's public API so it can be swapped into
the runtime import with no other code changes.
"""
...
CONFIG_KEY_POWER_METER_SHELLY = "shelly"
POWER_METER_TYPE_SHELLY = "shelly"
REQUEST_TIMEOUT = 5
...
def get_power_meter_reader(config_path: Optional[Path] = None) -> "PowerMeterReader":
    ...
    The current implementation selects a built-in Shelly reader from config.
    """
...
class ShellyPowerMeterReader(PowerMeterReader):
    """Read a Shelly HTTP JSON endpoint and normalize the payload."""
    ...
    def __init__(self, config_path: Optional[Path] = None, meter_config: Optional[dict] = None):
        ...
        if not isinstance(meter_config, dict):
            raise ValueError("powerMeter.shelly configuration is required and must be an object")

        self.power_meter_ip = meter_config.get("ip")
        if not self.power_meter_ip:
            raise ValueError("powerMeter.shelly.ip is required")
    ...
    def read(self) -> Optional[dict]:
        ...
        if total_power is not None:
            try:
                total_power = int(round(float(total_power)))
...
def build_power_meter_reader(config_path: Optional[Path] = None) -> PowerMeterReader:
    """Build the configured Shelly power-meter reader."""
    resolved_config = Path(config_path).resolve() if config_path is not None else _find_config_file().resolve()
    ...
    if meter_type != POWER_METER_TYPE_SHELLY:
        raise ValueError(
            f"Unsupported powerMeter.type '{meter_type}'. Supported types: ['{POWER_METER_TYPE_SHELLY}']"
        )

    shelly_config = power_meter_config.get(CONFIG_KEY_POWER_METER_SHELLY)
    return ShellyPowerMeterReader(config_path=resolved_config, meter_config=shelly_config)
