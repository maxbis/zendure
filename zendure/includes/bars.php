    ...
    if ($range <= 0) {
        ...
        switch ($scaleType) {
            ...
            case 'log':
                ...
                // Adjust for logarithmic scaling
...
/**
 * Calculate bidirectional bar properties for grid status display
 ...
 */
function getBidirectionalBarData($value, $min, $max) {
    ...
    if ($clampedValue > 0) {
        // Importing - bar extends to the right from center
        $barWidth = abs($clampedValue) / abs($max) * 50; // 50% max (half the container)
        ...
        // Exporting - bar extends to the left from center
        $barWidth = abs($clampedValue) / abs($min) * 50; // 50% max (half the container)
        // Ensure minimum width of 2% for visibility
        $barWidth = max(2, $barWidth);
        $barClass = 'exporting';
        $barValue = abs($actualValue) . ' W';
