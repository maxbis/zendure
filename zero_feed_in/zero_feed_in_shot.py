#!/usr/bin/env python3
"""
Single-shot P1 Meter Zero Feed-In Controller

Runs once:
- Reads current grid power from the Zendure P1 meter
- Reads current charge/discharge limits from zendure_data.json
- Calculates new limits to move grid power toward zero in one step
- Optionally sends the new limits to the Zendure device

Can be used both as:
- A command-line tool
- A library: Use set_power() for unified power control, or calculate_zero_feed_in() for detailed results
"""
...
# Path to config.json (can be overridden via CLI or library parameter)
CONFIG_FILE_PATH = Path(__file__).parent.parent / "zendure" / "config" / "config.json"
...
def _read_zendure_data() -> Optional[dict]:
    """
    Internal helper to read the zendure_data.json file.
    """
