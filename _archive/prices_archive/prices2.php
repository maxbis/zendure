...
// Schedule constants for netzero values
define('NETZERO_VALUE', -200); // Watts for "netzero" string
define('NETZERO_PLUS_VALUE', 400); // Watts for "netzero+" string

// Function to convert schedule value to numeric
function convertScheduleValue($value) {
    if ($value === 'netzero') return NETZERO_VALUE;
    if ($value === 'netzero+') return NETZERO_PLUS_VALUE;
...
// Fetch schedule data for each date
$scheduleData = [];
$scheduleApiBaseUrl = 'http://www.wijs.ovh/zendure/data/api/data_api.php?type=schedule&resolved=1';

foreach ($dates as $date) {
    $scheduleUrl = $scheduleApiBaseUrl . '&date=' . urlencode($date);
    ...
    if ($scheduleJson !== false) {
        $scheduleResponse = json_decode($scheduleJson, true);
        if ($scheduleResponse && isset($scheduleResponse['success']) && $scheduleResponse['success'] && isset($scheduleResponse['resolved'])) {
            $scheduleData[$date] = $scheduleResponse['resolved'];
        } else {
            $scheduleData[$date] = [];
        }
    } else {
...
foreach ($dates as $dateIndex => $date) {
    ...
    foreach ($hours as $hour) {
        ...
        // Find matching schedule value for this hour
        $scheduleValue = 0;
        if (isset($scheduleData[$date]) && is_array($scheduleData[$date])) {
            // Find the most recent schedule entry that applies to this hour
            // Schedule entries have 'time' in HHmm format (e.g., "0900")
            $hourTime = $hour . '00'; // Convert "09" to "0900"
            $activeEntry = null;
            
            foreach ($scheduleData[$date] as $entry) {
                if (isset($entry['time']) && $entry['time'] <= $hourTime) {
                    // Find the most recent entry (entries are already sorted by time)
                    $activeEntry = $entry;
                } else {
                    break; // Since entries are sorted, we can break early
                }
            }
            
            if ($activeEntry && isset($activeEntry['value'])) {
                $scheduleValue = convertScheduleValue($activeEntry['value']);
            }
        }
        $scheduleValues[] = $scheduleValue;
    }
}
...
if (count($dates) >= 2) {
    ...
    if ($currentDate == $secondDayDate && $currentHour >= 15) {
        ...
        if (file_exists($getPricesPath)) {
            ...
            // Reload schedule data for updated dates
            $scheduleData = [];
            foreach ($dates as $date) {
                $scheduleUrl = $scheduleApiBaseUrl . '&date=' . urlencode($date);
                $context = stream_context_create([
...
// Create color arrays for schedule line (green for positive, red for negative)
$scheduleBorderColors = [];
$scheduleBackgroundColors = [];
if (isset($scheduleValues)) {
    foreach ($scheduleValues as $value) {
        if ($value >= 0) {
            $scheduleBorderColors[] = 'rgba(76, 175, 80, 1)'; // green
            $scheduleBackgroundColors[] = 'rgba(76, 175, 80, 0.2)'; // green transparent
        } else {
            $scheduleBorderColors[] = 'rgba(244, 67, 54, 1)'; // red
            $scheduleBackgroundColors[] = 'rgba(244, 67, 54, 0.2)'; // red transparent
        }
    }
}

// Create datasets with price bars and schedule line
$scheduleDataset = [];
if (isset($scheduleValues) && count($scheduleValues) > 0) {
    $scheduleDataset = [
        'label' => 'Schedule (W)',
        'type' => 'line',
        'data' => $scheduleValues,
        'borderColor' => $scheduleBorderColors,
        'backgroundColor' => $scheduleBackgroundColors,
        'borderWidth' => 2,
        'fill' => true,
        'tension' => 0.1,
        'yAxisID' => 'y1',
...
// Add schedule dataset if available
if (!empty($scheduleDataset)) {
    $datasets[] = $scheduleDataset;
}
?>
<!DOCTYPE html>
<html lang="en">
                        ...
                        callbacks: {
                            ...
                            label: function(context) {
                                if (context.parsed.y !== null) {
                                    ...
                                    } else if (datasetLabel.includes('Schedule')) {
